...
/**
 * Chromium golden-file conformance suite. Reproduces each inspector-protocol
 * accessibility golden against our ax-tree.ts and asserts BYTE-EQUALITY with
 * the corresponding `-expected.txt` (URLs masked; the three goldens containing
 * ceiling-only content — InlineTextBox leaves and cross-frame iframe sections —
 * are diffed to /tmp/ax-audit but not asserted). The harness (testRunner.log
 * formatting, the accessibility-dumpAccessibilityNodes.js flow, tree rebuilds)
 * is a faithful port of web_tests/http/tests/inspector-protocol/{resources,
 * accessibility}. Requires the local Chromium checkout (see the memory note
 * "chromium-source-checkout"); skips itself when absent. Actual/expected pairs
 * land in /tmp/ax-audit/<name>.{actual,expected,notes}.txt for diffing.
 */
...
const OUT_DIR = "/tmp/ax-audit";
fs.mkdirSync(OUT_DIR, { recursive: true });

// The suite needs the local Chromium checkout with the golden files; skip
// everywhere else (CI) rather than fail.
const test = fs.existsSync(GOLDEN_DIR) ? vitestTest : vitestTest.skip;
...
/** URLs depend on the serving host (jsdom vs Chromium's test server) — mask them. */
...
class Harness {
  ...
  /**
   * Write the actual/expected pair for diffing, and — unless the golden
   * contains ceiling-only content (InlineTextBox leaves, cross-frame iframe
   * sections) — assert byte-equality with the Chromium golden.
   */
  finish(assertEqual = true): void {
    const body = [this.title, ...this.logs].join("\n") + "\n\n";
    fs.writeFileSync(path.join(OUT_DIR, `${this.name}.actual.txt`), body);
    const golden = path.join(GOLDEN_DIR, `${this.name}-expected.txt`);
    if (!fs.existsSync(golden)) return;
    fs.copyFileSync(golden, path.join(OUT_DIR, `${this.name}.expected.txt`));
    fs.writeFileSync(
      path.join(OUT_DIR, `${this.name}.notes.txt`),
      this.notes.length ? this.notes.join("\n") + "\n" : "",
    );
    if (assertEqual) expect(maskUrls(body)).toBe(maskUrls(fs.readFileSync(golden, "utf8")));
  }
}
...
function rewriteRelatedNodeValue(
  h: Harness,
  options: Options,
  value: Protocol.Accessibility.AXValue,
): void {
  if (!value.relatedNodes) {
    h.note(`Could not find relatedNodes in ${JSON.stringify(value)}`);
    return;
  }
...
function rewriteRelatedNodes(
  ...
  for (const node of nodes) {
    ...
    if (!properties)
      h.note(
        `Could not find ${node.ignored ? "ignoredReasons" : "properties"} in node role=${node.role?.value} name=${node.name?.value}`,
      );
    for (const source of node.name?.sources ?? []) {
      ...
      if (value.type === "idrefList" || value.type === "idref" || value.type === "nodeList")
        rewriteRelatedNodeValue(h, options, value);
    }
    for (const property of properties ?? []) {
      if (
        ...
        rewriteRelatedNodeValue(h, options, property.value);
    }
...
function dumpNode(h: Harness, nodes: AXNode[]): void {
  if (nodes.length !== 1) h.note(`dumpNode: expected exactly one node, got ${nodes.length}`);
  const node = nodes[0];
  if (!node) return;
...
function dumpBySelector(
  ...
  const h = new Harness(name, title);
  for (const el of Array.from(document.querySelectorAll(selector))) {
    ...
    if (fetchRelatives) {
      ...
      rewriteRelatedNodes(h, options, resp, el);
...
function printNodes(h: Harness, nodes: AXNode[], tolerateDangling = false): void {
  function printNodeAndChildren(node: AXNode, leadingSpace = ""): string {
    if (node.ignored) {
  ...
  for (const node of nodeMap.values()) {
    ...
    for (const childId of node.childIds ?? []) {
      ...
      if (!child) {
        if (!tolerateDangling) h.note(`dangling childId ${childId}`);
        continue;
      }
      child.parent = node;
      (node.children as AXNode[]).push(child);
...
test("accessibility-getFullAXTree", () => {
  const options = setup(
  ...
  const h = new Harness("accessibility-getFullAXTree", "Tests Accessibility.getFullAXTree");
  printNodes(h, roundtrip(getFullAXTree(options).nodes) as AXNode[]);
  h.note("iframe-subtree half of the golden skipped: cross-frame ceiling");
  h.finish(false); // ceiling: InlineTextBox leaves + iframe subtree
});

test("getFullAXTree-depth", () => {
  const options = setup(
    `<div>Some text in a div, also a <a href='https://www.example.com'>link</a></div>
    <button>Hello Button</button>`,
  );
  const h = new Harness("getFullAXTree-depth", "Tests Accessibility.getRootAXNode");
  printNodes(h, roundtrip(getFullAXTree(options, 2).nodes) as AXNode[], true);
  h.finish();
});

test("accessibility-getRootNode", () => {
  ...
  const h = new Harness("accessibility-getRootNode", "Tests Accessibility.getRootAXNode");
  ...
  h.note("iframe half of the golden skipped: cross-frame ceiling");
  h.finish(false); // ceiling: iframe half
});

test("accessibility-getChildAXNodes", () => {
  const options = setup(
    `<main>
    <article>
      <h1>Article</h1>
  ...
  const h = new Harness("accessibility-getChildAXNodes", "Tests Accessibility.getChildAXNodes");
  ...
  if (!article) {
    h.note("could not locate article via children[0]^4 (tree shape differs)");
  } else {
  ...
  h.note("iframe getChildAXNodes section skipped: cross-frame ceiling");
  h.finish(false); // ceiling: iframe section
});

test("accessibility-getAXNodeAndAncestors", () => {
  const options = setup(
    `<main>
    <article>
      <h1>Article</h1>
...
test("accessibility-getNodeWithNoAXNode", () => {
  dumpBySelector(
    "accessibility-getNodeWithNoAXNode",
...
test("accessibility-getTreeWithNoAXNode", () => {
  dumpBySelector(
    "accessibility-getTreeWithNoAXNode",
...
test("accessibility-getRelationships", () => {
  dumpBySelector(
    "accessibility-getRelationships",
...
test("accessibility-ignoredNodes", () => {
  dumpBySelector(
    "accessibility-ignoredNodes",
    "Tests accessibility values of ignored nodes.",
    // <html data-dump> in the golden merges onto the document element; we apply
    // it via mutate below. Inner content is otherwise verbatim.
...
test("accessibility-ignoredNodesModal", () => {
  dumpBySelector(
    "accessibility-ignoredNodesModal",
...
test("accessibility-aria-1.1", () => {
  dumpBySelector(
    "accessibility-aria-1.1",
...
test("accessibility-nameSources-buttons", () => {
  dumpBySelector(
    "accessibility-nameSources-buttons",
...
test("accessibility-nameSources-img-figure", () => {
  dumpBySelector(
    "accessibility-nameSources-img-figure",
...
test("accessibility-nameSources-input-buttons", () => {
  dumpBySelector(
    "accessibility-nameSources-input-buttons",
...
test("accessibility-nameSources-input", () => {
  dumpBySelector(
    "accessibility-nameSources-input",
...
test("accessibility-nameSources-labelledby", () => {
  dumpBySelector(
    "accessibility-nameSources-labelledby",
...
test("accessibility-nameSources-options", () => {
  dumpBySelector(
    "accessibility-nameSources-options",
...
test("accessibility-nameSources-summary", () => {
  dumpBySelector(
    "accessibility-nameSources-summary",
...
test("accessibility-nameSources-visiblity", () => {
  dumpBySelector(
    "accessibility-nameSources-visiblity",
...
test("accessibility-query-axtree", () => {
  const options = setup(
  ...
  getFullAXTree(options);
  const h = new Harness("accessibility-query-axtree", "Test finding DOM nodes by accessible name");
